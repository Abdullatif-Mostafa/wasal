...
import { FaUserCircle, FaEnvelope, FaEdit, FaSignOutAlt, FaMoon, FaSun, FaCog, FaTimes, FaUserFriends, FaArrowRight } from 'react-icons/fa';
...
export default function Page() {
  ...
  const handleEditChange = e => {
    ...
    if (name === 'profileImage' && files?.[0]) {
      ...
      setEditData(prev => ({ ...prev, profileImage: files[0] }));
    } else {
  ...
  const handleEditSubmit = e => {
    ...
    dispatch(editUserByIdAsync({ userId, data: formData }))
      .unwrap()
      .then(() => {
      ...
      <div className="w-full max-w-2xl bg-white/90 rounded-3xl shadow-2xl py-8 px-4 sm:px-3 border border-cyan-100 backdrop-blur-md flex flex-col items-center gap-6 relative">
        ...
        <div className="flex flex-col items-center gap-2">
          <div className="w-28 h-28 rounded-full bg-gradient-to-br from-teal-400 to-cyan-400 flex items-center justify-center text-6xl text-white shadow-lg overflow-hidden">
            {selectedUser?.image ? (
              <img src={selectedUser?.image} alt="الصورة الشخصية" width={112} height={112} className="object-cover rounded-full" />
            ) : (
              <FaUserCircle />
            )}
          </div>
          <h2 className="text-3xl font-extrabold text-teal-700 mt-2">{selectedUser?.username}</h2>
          <p className="text-cyan-700 text-sm flex items-center gap-2"><FaEnvelope /> {selectedUser?.email}</p>
          <p className="text-gray-600 text-sm mt-1">عضو منذ: {selectedUser?.joined}</p>
        </div>
        {/* Stats Grid */}
        ...
        <div className="w-full bg-cyan-50 rounded-xl shadow p-4 flex flex-col gap-2">
          ...
          <p className="text-gray-700 text-sm">{selectedUser?.bio || 'لا يوجد  حاليا نبذة شخصية'}</p>
        </div>
            ...
            <ul className='flex flex-col gap-0.5'>
              {posts?.map((post) => {
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center px-3 pt-3 pb-2 border-b bg-teal-50 rounded-t-2xl">
                      <Link href={`/Pages/${post?.user?._id}`} className="flex items-center justify-center gap-2">
                        <span className="w-10 h-10 flex justify-center items-center rounded-full bg-gradient-to-br from-teal-400 to-cyan-400  text-xl font-bold text-white shadow-md">
