    ...
    <div className="max-w-2xl mx-auto bg-white/90 rounded-3xl shadow-2xl p-8 border border-cyan-100 backdrop-blur-md flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        {notifications.map((notif) => (
          <Link key={notif.id} href={notif.link} className="flex items-center gap-4 bg-cyan-50 rounded-xl shadow p-4 hover:bg-cyan-100 transition group relative overflow-hidden border-l-4 border-cyan-300 group-hover:border-teal-400">
            <span className="flex-shrink-0 group-hover:scale-110 transition-transform">
              {notif.icon}
            </span>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="font-bold text-teal-800 group-hover:text-cyan-600 transition-colors">{notif.user}</span>
