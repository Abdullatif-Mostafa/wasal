// Posts.js
"use client";
import { addLike, deletePostAsync, fetchPosts } from "@/RTK/Reducers/postSlice";
...
const Posts = ({ newPost }) => {
  ...
  // هل المستخدم أعجب بهذا البوست؟
  // const hasLiked = useMemo(() => {
  //   console.log("posts.likes", posts.likes);
  //   console.log("user._id", user?._id);
  //   return posts.likes?.includes(user?._id);
  ...
  const handleLike = (post) => {
    ...
    console.log("post in the posts ", post);
    if (!isAuthenticated || !user) {
      toast.error("يجب تسجيل الدخول للإعجاب بالمنشور", { rtl: true });
      return;
    }
    // console.log("posts._id", post._id);
    // console.log("user._id", user._id);
    dispatch(addLike({ postId: post._id, token }))
      .unwrap()
      .then((data) => {
        console.log("Liked post in Posts:", data);
        // يرجع البوست المحدث
      ...
      .catch((error) => {
        console.error("فشل في الإعجاب بالمنشور:", error);
      });
  };
          ...
          return (
            ...
            >
              ...
              <div className="flex justify-between gap-2 border-t pt-3 px-6 pb-4 bg-cyan-50 rounded-b-2xl">
                <Link href={'#'} onClick={() => handleLike(post)} type="button"
                  className={`${hasLiked ? "text-red-500" : "text-cyan-700"} flex items-center gap-1 px-3 py-1 rounded-lg hover:bg-cyan-100 font-semibold text-cyan-700 cursor-pointer transition-colors`}
                >
                  <span className="chakra-button__icon">
                ...
                <button type="button" className="flex items-center gap-1 px-3 py-1 rounded-lg hover:bg-cyan-100 font-semibold text-cyan-700 cursor-pointer transition-colors">
                  <span className="chakra-button__icon">
