...
import { FaImage, FaSmile, FaPaperPlane, FaVideo } from "react-icons/fa";
...
const CreatePost = ({ onPost }) => {
  ...
  const handlePostSubmit = async () => {
    ...
    if (!postText && !image && !video) {
      toast.error("يرجى إضافة نص أو صورة او فيديو للمنشور", { rtl: true });
      return;
    }
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-gradient-to-br from-cyan-50 via-white to-teal-100 rounded-2xl shadow-lg p-5 mb-0 mt-2.5 border border-cyan-200">
      ...
      {preview && (
        <div className="mb-3">
          {image && <Image src={URL.createObjectURL(image)} alt="معاينة الصورة" width={400} height={256} className="rounded-lg max-h-64 object-cover border border-cyan-200 mx-auto" />}
          {video && (
            <video src={URL.createObjectURL(video)} controls className="rounded-lg max-h-70 w-70 object-cover border border-cyan-200 mx-auto" />
          )}
        </div>
      )}
      <div className="flex items-center justify-between gap-2 mt-2">
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer text-cyan-700 hover:text-teal-600 transition">
            ...
            <span className="text-sm font-semibold">إضافة فيديو</span>
